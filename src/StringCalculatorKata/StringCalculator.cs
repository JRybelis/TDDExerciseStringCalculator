...
namespace StringCalculatorKata
{
    public class StringCalculator
    {
        public static int Add(string numbers)
        {
            ...
            return rest.Split(delimiters, StringSplitOptions.None).Select(ParseOrZero).Sum();
        }
        ...
        private static (string[], string) ExtractDelimitersAndNumbers(string numbers)
        {
            ...
            numbers = parts[1];
            var candidateDelimiters = parts[0].Replace("/", "");
        ...
        {
            var failure = !int.TryParse(candidate, out var result);
